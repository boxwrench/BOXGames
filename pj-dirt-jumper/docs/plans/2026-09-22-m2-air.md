...
- **Deviations from the spec agreed with the user in chat (2026-09-22):**
  ...
  5. *(added during Task 3 calibration)* Lip magnetism: takeoff horizontal speed is nudged by up to ±30% so the arc
     lands on the jump's sweet spot; lift (speed + pop) still sets airtime and landing angle.
  6. *(added during Task 3 calibration)* Pumping and landing bonuses cap at `maxSpeed`; gravity can carry PJ to
     `hardSpeed`. Landings climb back `climbBack` (60%) of their drop, so the trail descends gently (~5%).
  7. *(added during Task 3 calibration)* Air tuned for long hang time with sane apexes: `airGravity 9`, `popBoost 3`,
     `perfectPopBonus 1.2`, takeoff angles 23–37°.

> **Calibration note (Task 3):** the jump builder in Step 4 below is the *starting* design. The committed version
> differs: landing ramps are the bot's flight path lowered by `rampClear·u² + rampCross·u` (u = distance to the sweet
> spot / ramp length) so slower riders clear the deck and the bot lands within 0.1 m; decks are chosen first
> (`deck` range relative to the lip) and the ramp start found on the arc; `Jump.aim` carries the sweet-spot height and
> the good/perfect pop lift for steering. See `src/track/generate.ts` and `git log` for the reasoning.
